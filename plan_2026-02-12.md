...
- New in this pass:
  - Added explicit `--baseline-mode {run,skip}` control with backwards-compatible `--skip-nanochat-baseline` alias; skip now records `status: skipped` with a reason in JSON and markdown outputs.
  - Baseline timeout limitation remains expected on this host under tight runtime budgets (`120s`/`180s`).
  - Updated Colab/profile launch paths to pass baseline controls and timeout through `BASELINE_MODE`, `SKIP_NANOCHAT_BASELINE`, and `NANOCHAT_TRAIN_TIMEOUT_SEC`.
  - Extended `notebooks/colab_gpu_benchmark.ipynb` env passthrough for `BASELINE_MODE` / `NANOCHAT_TRAIN_TIMEOUT_SEC` to avoid per-machine edits before moving to GPU hosts.

### Colab handoff checklist (next machine)
- Recommended baseline run matrix (from repo root):
  - Sanity / smoke run (no baseline):
    - `BASELINE_MODE=skip REQUIRE_GPU=0 PROFILE=quick bash scripts/colab_gpu_benchmark.sh`
  - Minimal quick comparison:
    - `BASELINE_MODE=run NANOCHAT_TRAIN_TIMEOUT_SEC=300 NANOCHAT_NUM_ITERATIONS=40 EXTRA_ARGS="--nanochat-max-chars 200000 --nanochat-eval-every 10" REQUIRE_GPU=1 PROFILE=quick bash scripts/colab_gpu_benchmark.sh`
  - Full comparison:
    - `BASELINE_MODE=run NANOCHAT_TRAIN_TIMEOUT_SEC=1800 REQUIRE_GPU=1 PROFILE=full bash scripts/colab_gpu_benchmark.sh`
- Equivalent direct profile wrapper commands:
  - Skip baseline: `BASELINE_MODE=skip BASELINE=nanochat REQUIRE_GPU=1 PROFILE=quick bash scripts/profile_gpu.sh`
  - Run baseline: `BASELINE_MODE=run NANOCHAT_TRAIN_TIMEOUT_SEC=1200 BASELINE=nanochat REQUIRE_GPU=1 PROFILE=quick bash scripts/profile_gpu.sh`
- On completion, capture artifacts from:
  - `results/benchmark_karpathy_*.json`
  - `results/benchmark_karpathy_*.md`
  - `results/benchmark_karpathy_*_logs/*.log`
